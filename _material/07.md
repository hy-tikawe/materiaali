...
Yksi tapa testata sovelluksen toimintaa suurella tietomäärällä on luoda testidataa, jossa tietokannan tauluihin lisätään paljon tietoa. Tässä osassa testaamme keskustelualueen toimintaa tällaisella datalla.
...
Testidatan luomisen jälkeen ensimmäinen havainto sovelluksesta on, että yritys mennä etusivulle saa selaimen jumiutumaan. Syynä on, että tietokannassa on nyt sata tuhatta ketjua, jotka kaikki yritetään näyttää etusivulla.

Kun sovelluksessa on paljon tietoa, sovelluksen sivut kannattaa toteuttaa niin, että yhdellä sivulla näkyvän sisällön määrä on rajoitettu. Etusivulla hyvä ratkaisu on lisätä listaan sivutus, joka näyttää kerrallaan vain pienen osan ketjuista.
...
Tämä tarkoittaa, että sivulla on valinnainen parametri `page`, joka ilmaisee sivunumeron. Esimerkiksi osoite `/5` tarkoittaa sivua 5. Jos sivua ei anneta, se on oletuksena 1 eli osoitteet `/` ja `/1` tarkoittavat samaa. Lisäksi koodi varmistaa funktion `max` avulla, että sivu on aina vähintään 1.
...
Funktiossa muuttuja `page_size` määrittelee, montako ketjua näytetään yhdellä sivulla. Tässä on valittu, että yhdellä sivulla näytetään kymmenen ketjua. SQL-kyselyssä `LIMIT` on haettavien tulosrivien määrä ja `OFFSET` on ensimmäisen haettavan rivin kohta alkuperäisessä tulostaulussa.
...
Sivupohjassa sivutus näkyy siinä, että ketjulistan näytetään sivunumero sekä linkit, joiden avulla voi siirtyä edelliselle ja seuraavalle sivulle:
...
Tässä `&lt;` ja `&gt;` ovat HTML-entiteettejä, jotka vastaavat merkkejä `<` ja `>`. Koska näitä merkkejä käytetään myös HTML-elementeissä, on parempi kirjoittaa ne entiteetteinä.
...
Tästä näkee, että jokaisen sivupyynnön käsittely vie aikaa välillä 0.86–0.87 sekuntia. Tästä syntyy viive, joka aiheuttaa häiritsevän kokemuksen sovelluksen käyttäjälle.

## Tietokannan indeksit

Tyypillinen syy web-sovelluksen hitauteen on, että sovellus suorittaa raskaita tietokantakyselyjä. Usein helppo ratkaisu ongelmaan on lisätä tietokantaan indeksejä, jotka tehostavat kyselyitä.
...
Tässä kyselyssä tietokanta yhdistää tietoa tauluista `threads` ja `messages`, koska se ilmoittaa jokaisen ketjun kohdalla viestien määrän sekä uusimman viestin lähetysajan. Kysely toimii tällä hetkellä hitaasti, koska tietokannassa ei ole tehokasta tapaa selvittää ketjun id-numeron perusteella, mitä viestejä ketjussa on.
...
Lisätään vielä haastetta luomalla uusi suurempi testidata, jossa ketjujen ja viestien määrä on kymmenkertainen:
