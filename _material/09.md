...
Hyvä lähtökohta saavutettavan sovelluksen tekemiseen on käyttää HTML-elementtejä suoraviivaisesti ja loogisesti.

Jos sovelluksessa on kuvia, `alt`-attribuutilla voidaan antaa kuvan korvaava tekstisisältö. Voimme käyttää attribuuttia käyttäjäsivulla näin:
...
Lynx-selain näyttää alt-attribuutissa olevan tekstin näin:

![](../img/09-lynx3.png)

Lomakkeissa saavutettavuutta voi parantaa käyttämällä `label`-elementtiä, joka yhdistää lomakkeen kentän ja sen kuvauksen. Tarkastellaan esimerkkinä seuraavaa lomaketta:
...
Tässä "Viesti:" on tekstialueen kuvaus, mutta tämä ei tule ilmi koodista. Voimme nyt parantaa lomaketta käyttämällä `label`-elementtiä näin:
