...
Yksi hyvä tulikoe sovellukselle on testata sitä tekstipohjaisella selaimella. Seuraavassa on testattu keskustelualuetta Lynx-selaimella:

![](../img/09-lynx1.png)

![](../img/09-lynx2.png)

Tässä tapauksessa sovellusta pystyy käyttämään hyvin tekstiselaimessa, mikä on merkki hyvästä saavutettavuudesta. Yleensä hyvään tulokseen pääsee sillä, että tekee yksinkertaista HTML-koodia, jossa elementtejä on käytetty loogisesti.

Yksi saavutettavuutta parantava tekniikka on käyttää `label`-elementtiä lomakkeissa. Tarkastellaan esimerkkinä seuraavaa lomaketta:
...
Tässä "Viesti:" on tekstialueen otsikko, mutta tämä ei tule ilmi koodista. Lomaketta voi parantaa käyttämällä `label`-elementtiä näin:
