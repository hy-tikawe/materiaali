...
Lisäksi voimme toteuttaa kirjautumissivun niin, että se ohjaa käyttäjän automaattisesti takaisin sivulle, jolla käyttäjä oli ennen kirjautumista. Tätä varten laitetaan lomakkeeseen seuraava piilokenttä:

{: .code-title }
login.html
```jinja
    {% raw %}{% if request.referrer %}
    <input type="hidden" name="next_page" value="{{ request.referrer }}">
    {% else %}
    <input type="hidden" name="next_page" value="/">
    {% endif %}{% endraw %}
```

Piilokenttä `next_page` ilmaisee sivun, jonne käyttäjä ohjataan. Tässä `request.referrer` sisältää edellisen sivun osoitteen, jos selain on antanut tämän tiedon. Jos tietoa ei ole, käyttäjä ohjataan sovelluksen etusivulle.

Lomakkeen käsittelijä käyttää piilokenttää näin:
