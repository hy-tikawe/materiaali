...
Tällä hetkellä käyttäjä voi kirjoittaa lomakkeisiin mitä tahansa ja käyttäjän lähettämät tiedot tallennetaan tietokantaan ja näytetään sovelluksessa. Tämä voi aiheuttaa ongelmia, jos tiedon määrä on poikkeavan suuri tai pieni.
...
{: .note-title }
Suuren syötteen luominen
<div class="note" markdown="1">
Suuren syötteen luominen on kätevää Pythonin avulla. Esimerkiksi voimme luoda seuraavasti merkkijonon, jossa on tuhat a-kirjainta:
...
Korjaus ongelmaan on rajoittaa syötteen kokoa lomakkeiden käsittelyssä. Voimme käyttää tähän `maxlength`-attribuuttia:
...
Välillä voi olla tarpeen myös estää tyhjä syöte. Esimerkiksi jos käyttäjä lähettää ketjun tyhjällä otsikolla, listaan ei tule näkyvää linkkiä ketjuun:

![](../img/05-smallinput1.png)

![](../img/05-smallinput2.png)

Yksi mahdollinen korjaus on muuttaa tyhjä syöte joksikin muuksi:
...
Tämän jälkeen ketju näkyy järkevämmin listassa:

![](../img/05-smallinput3.png)

Nämä asiat kannattaa käydä läpi kaikissa sovelluksen lomakkeissa. Tyypillinen virhe on unohtaa rajoitus jossain tilanteessa. Esimerkiksi viestiä lähettäessä pituus voi olla rajoitettu mutta viestiä muokatessa ei ole rajoitusta.
