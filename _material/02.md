...
Koodi asettaa myös kentälle `db.row_factory` arvon `sqlite3.Row`, minkä ansiosta kyselyn tulostaulun sarakkeisiin pystyy viittaamaan sarakkeiden nimien avulla. Esimerkiksi jos kyselyn tulosrivi on muuttujassa `result` ja haluamme viitata ensimmäiseen sarakkeeseen `name`, voimme käyttää syntaksia `result["name"]` aiemmin käytetyn syntaksin `result[0]` lisäksi.
