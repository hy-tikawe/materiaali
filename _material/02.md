...
Funktio `query` on tarkoitettu suorittamaan tietoa hakeva `SELECT`-komento. Funktio hakee kaikki tulosrivit metodilla `fetchall` ja palauttaa ne. Lisäksi funktion `query_one` avulla kyselyn tuloksen saa haettua mukavammassa muodossa, kun kyselyssä on aina tasan yksi tulosrivi (kuten kysely muotoa `SELECT COUNT(*)`).
...
Tämän virheilmoituksen perusteella on usein hyvin vaikeaa selvittää virheen syytä, koska virheestä ei näytetä tarkempaa tietoa. Yleensä kuitenkin virheen syy selviää tutkimalla lokia, jonka sovellus on tulostanut komentoikkunaan.
...
Yleensä lokin lopussa on hyödyllistä tietoa virheen selvittämiseen liittyen. Näin on myös tässä tapauksessa: loki kertoo, että virhe tapahtuu tiedostossa `app.py` rivillä 8, jossa suoritetaan komento `db.execute`:
...
Yleisemminkin nämä ovat tietoja, joita kannattaa yrittää selvittää lokista: millä rivillä virhe tapahtuu ja onko jotain virheviestiä, joka kertoo virheen syyn.
...
Voimme esimerkiksi lisätä keskustelualueeseen seuraavan debug-tulostuksen, joka näyttää lokissa kaiken tietokannasta haetun tiedon, kun käyttäjä menee sovelluksen etusivulle:
